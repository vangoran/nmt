...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ### END YOUR CODE
        padded_source_embedding = self.model_embeddings.source(source_padded)
        X = pack_padded_sequence(padded_source_embedding, source_lengths) # (src_len, b, e)
        output, (hn, cn) = self.encoder(X)
        enc_hiddens, _ = pad_packed_sequence(output, total_length=padded_source_embedding.shape[0])
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_split = torch.split(Y, 1) #(1, b, e)

        for item in Y_split:
            Y_t = torch.squeeze(item) #(b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        h_decode_t = torch.unsqueeze(dec_hidden, dim=2) #(b, h, 1)
        e_t = torch.bmm(enc_hiddens_proj, h_decode_t) #(b, src_len, 1)
